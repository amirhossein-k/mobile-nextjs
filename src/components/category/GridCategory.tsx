...
const variants = cva(" rounded-3xl    flex flex-col justify-center    ", {
  variants: {
    size: {
      "1x1": " sm:col-span-2 shadow-shadow-catmain h-full  cursor-pointer",
      ...
      "5x5": "col-span-8  row-span-auto w-full  md:col-span-6  lg:col-span-5 ",
      "8x5": "col-span-8  row-span-auto w-full  md:col-span-8  lg:col-span-8 ",
    },
  },
  defaultVariants: {
    size: "1x1",
...
const GridCategoryItem = ({size, children}: GridItemProps) => {
  return <div className={cn(variants({size, className: ""}))}>{children}</div>;
};

export default GridCategoryItem;
